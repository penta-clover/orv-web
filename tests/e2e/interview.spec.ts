...
test.describe("인터뷰 진행", () => {
  test("인터뷰 주제 선택 및 진행", async ({ page }) => {
    // 인터뷰 주제 선택
    ...
    // 인터뷰 제목 설정
    await page.getByRole('textbox', { name: '인터뷰 제목을 입력해주세요' }).click();
    await page.getByRole('textbox', { name: '인터뷰 제목을 입력해주세요' }).fill('(auto) 테스트 인터뷰');
    ...
    // 완료 후 홈으로 이동
    await page.getByRole('button', { name: 'left arrow 홈으로 돌아가기' }).click();
  });
});
