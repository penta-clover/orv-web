...
function topicMapper(topic: string) {
  switch (topic) {
    ...
    case "슬픔":
      return `답장을 보내면서 그 때 당시 나의 얼굴이 떠올랐어. 너가 최근에 겪은 그 슬픔, 누구에게도 말하지 못했던 그 감정 사실 나도 아직 완전히 이겨내진 못했어... 하지만 그 아픔 덕분에 내가 더 단단해질 수 있었던 것 같아. 그러니까, 지금의 너도 기억해줘. 이 시기를 버텨낸 너는 결국 빛나는 사람이 될 거라는 것을.. 아 참! 요즘 내가 어떻게 지내는 지 궁금하지 않아??`;
    case "분노":
      return `그때의 나를 생각하니, 그때의 감정이 다시 떠오르네. 그 감정이 나를 지배하고 있었던 것 같아. 하지만 지금은 그 감정을 잘 다스릴 수 있게 되었어. 그때의 나에게 고마워. 아 참! 요즘 내가 어떻게 지내는지 들려줄게`;
    default:
